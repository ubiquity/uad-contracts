...
describe("Pause V2 Bonding", () => {
  ...
  describe("Pausing BondingV2", () => {
    ...
    it("Should pause deposit, addLiquidity and removeLiquidity", async () => {
      ...
      await bondingV2.connect(admin).pause();
      await expect(
        bondingV2.connect(secondAccount).addLiquidity(bondId, amount, 1)
      ).to.be.revertedWith("Pausable: paused");
      ...
      await bondingV2.connect(admin).pause();
      await expect(
        bondingV2.connect(secondAccount).removeLiquidity(bondId, amount)
      ).to.be.revertedWith("Pausable: paused");
  ...
  describe("Pausing BondingShareV2", () => {
    it("Should pause and unpause only with role", async () => {
      ...
      // pause revert if already paused
      await expect(bondingShareV2.connect(admin).pause()).to.be.revertedWith(
        "Pausable: paused"
      );
      ...
      // setting role to pause and unpause
      await manager.connect(admin).grantRole(PAUSER_ROLE, secondAccountAddress);
      await expect(bondingShareV2.connect(secondAccount).pause()).to.not.be
        .reverted;
      await expect(bondingShareV2.connect(secondAccount).unpause()).to.not.be
        .reverted;
    });
    ...
    it("Should pause updateBond, mint, safeTransferFrom and safeBatchTransferFrom", async () => {
      ...
      await expect(
        bondingShareV2.connect(admin).updateBond(1, 1, 1, 1)
      ).to.be.revertedWith("Pausable: paused");

      await expect(
        bondingShareV2.connect(admin).mint(secondAccountAddress, 1, 1, 1)
      ).to.be.revertedWith("Pausable: paused");
  ...
  describe("Pausing BondingV2 and BondingShareV2", () => {
    it("Should not pause BondingV2 pendingLpRewards, lpRewardForShares, currentShareValue and sendDust", async () => {
      ...
      await expect(bondingV2.connect(secondAccount).pendingLpRewards(1)).to.not
        .be.reverted;

      await expect(bondingV2.connect(secondAccount).lpRewardForShares(1, 1)).to
        .not.be.reverted;

      await expect(bondingV2.connect(secondAccount).currentShareValue()).to.not
        .be.reverted;
    ...
    it("Should pause BondingV2 migrate", async () => {
      ...
      await metaPool
        .connect(secondAccount)
        .approve(bondingV2.address, BigNumber.from(10));
