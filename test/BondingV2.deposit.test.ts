...
describe("deposit", () => {
  ...
  describe("pendingLpRewards", () => {
    ...
    it("should increase only when we deposit after inflation ", async () => {
      ...
      await twapOracle.update();
      const uADPrice2 = await twapOracle.consult(uAD.address);
      console.log(` uADPrice2: ${ethers.utils.formatEther(uADPrice2)}`);
      ...
      console.log(
        `
      fourthAccount lp balance:${ethers.utils.formatEther(
        await metaPool.balanceOf(await fourthAccount.getAddress())
      )}
      fourthAccount crv balance:${ethers.utils.formatEther(
        await crvToken.balanceOf(await fourthAccount.getAddress())
      )}
      `
      );
      await swap3CRVtoUAD(metaPool, crvToken, one.mul(30000), fourthAccount);
      ...
      await swap3CRVtoUAD(metaPool, crvToken, one.mul(100), fourthAccount);
      await twapOracle.update();
      const uADPrice3 = await twapOracle.consult(uAD.address);
      console.log(` uADPrice3: ${ethers.utils.formatEther(uADPrice3)}`);
      ...
      console.log(`
      bondingV2Bal2After:${ethers.utils.formatEther(bondingV2Bal2After)}
      bondingV2BalAfter:${ethers.utils.formatEther(bondingV2BalAfter)}
      `);
      ...
      console.log(`
      bondingV2Bal2After:${ethers.utils.formatEther(bondingV2Bal2After)}
      totalLPAfter2ndDeposit:${ethers.utils.formatEther(totalLPAfter2ndDeposit)}
      pendingLpRewards1After2ndExcessDollarDistrib:${ethers.utils.formatEther(
        pendingLpRewards1After2ndExcessDollarDistrib
      )}
      pendingLpRewards2After2ndExcessDollarDistrib:${ethers.utils.formatEther(
        pendingLpRewards2After2ndExcessDollarDistrib
      )}
      `);
