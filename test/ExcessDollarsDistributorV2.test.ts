...
describe("ExcessDollarsDistributorV2", () => {
  ...
  it("distributeDollars should work", async () => {
    ...
    const dyuADto3CRV = await metaPool["get_dy(int128,int128,uint256)"](
      0,
      1,
      amount.sub(fiftyPercent).sub(tenPercent)
    );
    ...
    const UGOVPair = (await ethers.getContractAt(
      "IUniswapV2Pair",
    ...
    expect(
      uGOVLPTotalSupplyAfterDistribution.sub(
        uGOVLPTotalSupplyBeforeDistribution.add(zeroAdrBalAfterBurnLiquidity)
      )
    ).to.be.lt(ethers.utils.parseEther("0.0004"));
    expect(zeroAdrBalAfterBurnLiquidity).to.be.gt(
      zeroAdrBalBeforeBurnLiquidity
    );
    const scBalAfterBurnLiquidity = await UGOVPair.balanceOf(
      excessDollarsDistributor.address
    );
    ...
    const fourtyPercentAsLP = currentLPInBonding.sub(
      LPInBondingBeforeDistribute
    );
