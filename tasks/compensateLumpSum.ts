...
task("compensateLumpSum", "compensate incorrect staking bond lump sum")
    ...
    async (
      ...
      const applyMultiplier = async (_address: string, bondId: number) => {
        ...
        try {
          ...
          const pendingUGOVWithoutRewardDebt =
            pendingUGOV.add(currentLpRewardDebt);
          const multiplierCompensate = correctMultiplier.sub(
            BigNumber.from("1000000000000000000")
          );

          const lumpSumCompensation = pendingUGOVWithoutRewardDebt
            .mul(multiplierCompensate)
            .div(BigNumber.from("1000000000000000000"));

          const treasuryTwentyPercent = lumpSumCompensation
          ...
          console.log(
            "Pending UGOV without reward debt (full reward from bond creation until now)",
            pendingUGOVWithoutRewardDebt.toString(),
            ethers.utils.formatEther(pendingUGOVWithoutRewardDebt)
          );
          console.log(
          ...
          console.log(
            "Lump sum UBQ compensation",
            lumpSumCompensation.toString(),
            ethers.utils.formatEther(lumpSumCompensation)
          );
          console.log(
          ...
          const txBondHolder = await ubqToken
            .connect(admin)
            .mint(_address, lumpSumCompensation);
